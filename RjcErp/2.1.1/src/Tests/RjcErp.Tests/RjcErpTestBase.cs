...
namespace RjcErp.Tests
{
    public abstract class RjcErpTestBase : AbpIntegratedTestBase<RjcErpTestModule>
    {
        ...
        protected void UsingDbContext(Action<RjcErpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<RjcErpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<RjcErpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<RjcErpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<RjcErpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RjcErpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<RjcErpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RjcErpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<RjcErpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<RjcErpDbContext, Task<T>> func)
        {
            T result;
